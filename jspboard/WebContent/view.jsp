...
<%@ page import="bbs.HeartDAO" %>
<%@ page import="bbs.heart" %>
...
// 					List<Bbs> boardLists = bbs.selectListPage(param);
	%>
	<nav class="navbar navbar-default">
		...
		<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
			<ul class="nav navbar-nav">
				...
				if (boardID == 1){
				%>
					<li class="active"><a href="bbs.jsp?boardID=1&pageNumber=1">게시판</a></li>
					<li><a href="bbs.jsp?boardID=2&pageNumber=1">자유 게시판</a></li>
				<%
				} else if(boardID == 2){
				%>
					<li><a href="bbs.jsp?boardID=1&pageNumber=1">게시판</a></li>
			...
			<ul class="nav navbar-nav navbar-right">
        		...
        		<ul class="dropdown-menu">
              		<li><a href="heartBbs.jsp?pageNumber=1">찜목록</a></li>
		...
		<div class="container">
			<table class="table table-haver" style="border: 1px solid #dddddd">
					...
					<tr>
						...
						ArrayList<heart> list1 = heartDAO.getheart(userID, bbsID);
						if (list1.isEmpty()){
						%>
						<td align="right" bgcolor="beige"><button onclick = "location.href='heartAction.jsp?bbsID=<%=bbsID%>'">좋아요</button></td>
						<%	}
							else{%>
						<td align="right" bgcolor="beige"><button onclick = "location.href='heartAction.jsp?bbsID=<%=bbsID%>'">좋아요해제</button></td>
						<%	} %>						
					</tr>
					<%} 
					if (boardID == 2){%>
					...
					<tr>
						...
						<td colspan="3" align="right"><%= bbs.getBbsDate() %></td>
					</tr>
					<% 	
					String real = "C:\\DevData\\JSP&Servlet\\jspboard\\WebContent\\bbsUpload";
				...
				%>
					...
					<%
						...
						for(int i=0; i<list.size(); i++){
							...
							soso = soso + list.get(i).getsoso();
							bad = bad + list.get(i).getbad();
						} %>
					<tr align="center">
						<td colspan="2"><button onclick="location.href='evaluationAction.jsp?good=1&soso=0&bad=0&bbsID=<%=bbsID%>'">좋아요(<%= good %>)</button></td>		
						<td colspan="2"><button onclick="location.href='evaluationAction.jsp?good=0&soso=1&bad=0&bbsID=<%=bbsID%>'">그럭저럭(<%=soso %>)</button></td>		
						<td colspan="2"><button onclick="location.href='evaluationAction.jsp?good=0&soso=0&bad=1&bbsID=<%=bbsID%>'">별로에요(<%=bad %>)</button></td>		
					</tr>
					<tr>
					<%
						}
								...
								<div class="row">
									<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
										...
										<tr>
											...
											<% 	
												String commentReal = "C:\\DevData\\JSP&Servlet\\jspboard\\WebContent\\bbsUpload";
