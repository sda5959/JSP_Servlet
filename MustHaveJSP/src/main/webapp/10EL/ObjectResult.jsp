...
<body>
	...
	<!-- 
		...
		호출하여 값을 출력한다. 즉 아래문장은 getter()을 호출하므로 Person클래스
		에서 getName()을 주석처리하면 에러가 발생한다
	 -->
		<li>Person 객체 => 이름 : ${ personObj.name },
							나이 : ${ personObj.age }</li>
		<!--  request영역을 지정하는 requestScope는 생략할 수 있다. -->
		<li>String 객체 => ${ requestScope.stringObj }</li>
		<li>Integer 객체 => ${ integerObj }</li>
	</ul>
	
	<h2>JSP내장객체를 통해 영역의 값을 읽어오기</h2>
	<%
	// 영역에는 모든 객체를 저장할 수 있는 공간이므로 Object형으로 저장된다.
	Object object = request.getAttribute("personOjb");
	// 따라서 영역에서 객체를 꺼낸후 사용시에는 원래의 타입으로 형변환해야 한다.
	Person person =(Person) object;
	// 출력시에는 멤버변수에 직접 접근할 수 없으므로 getter()메서드를 사용한다.
