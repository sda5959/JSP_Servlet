...
<%-- 	</c:set> --%>
	...
	<!--  XML파일을 위해 parse태그에 1차로 설정한다. 뒤에서 blist라는 변수명
	으로 각 노드를 파싱할 것이다. -->
	<x:parse xml="${booklist}" var="blist" />
	
	<!--  booklist하위 노드에서 2개의 book이 있으므로 인덱스를 통해 
	접근한 후 데이터를 파싱한다. 이와같이 XML문서를 마치 경로처럼
	접근하는 것을 Xpath라고 한다. XPaht는 EL과 다르게 {}를 사용하지
	않는다. -->
	<h4>파싱 1</h4><!-- 파싱을 위해 xalan.jar파일이 필요 -->
	...
	<table border="1">
		...
		<tr>
			...
			<!--  if태그로 조건이 true일때만 실행되는 구문을 작성한다.
