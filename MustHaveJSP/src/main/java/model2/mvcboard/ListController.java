...
public class ListController extends HttpServlet {
   ...
   protected void doGet(HttpServletRequest req, HttpServletResponse resp)
      ...
      //전체 게시물의 갯수를 카운터하여 반환한다. 게시물의 갯수를 통해 가상번호를
      // 부여하거나 전체 페이지수를 계산한다.
      ...
      /*
         ...
         파라미터를 읽어온다. 
       	 한 페이지에 출력할 게시물의 갯수와 한 블럭당 출력할 페이지번호의 갯수이다.
      */
      ...
      /*
       	 목록에 첫 진입 시 무조건 1페이지로 가정한 후 게시물을 얻어온다.
       	 특정 페이지로 진입한 경우에는 파라미터를 받아서 구간을 계산한다.
      */
      int pageNum = 1;
      // 파마리터로 넘어오는 pageNum이 있다면 값을 얻어온다.
      String pageTemp = req.getParameter("pageNum");
      // 값이 있따면 정수로 변환한 후 페이지번호로 지정한다.
